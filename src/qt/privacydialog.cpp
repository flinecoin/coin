                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zFLINE ought to be enough for anybody." - Bill Gates, 2017
    ui->zFLINEpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzFLINESyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzFLINE->setEnabled(false);
        ui->pushButtonMintzFLINE->setToolTip(tr("zFLINE is currently disabled due to maintenance."));

        ui->pushButtonSpendzFLINE->setEnabled(false);
        ui->pushButtonSpendzFLINE->setToolTip(tr("zFLINE is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zFLINEpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzFLINE_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFLINE is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zFLINE...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zFLINE in ") +
...
void PrivacyDialog::on_pushButtonSpendzFLINE_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zFLINE is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zFLINE
        sendzFLINE();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zFLINE
    sendzFLINE();
}

void PrivacyDialog::on_pushButtonZFlineControl_clicked()
{
    ZFlineControlDialog* zFlineControl = new ZFlineControlDialog(this);
    zFlineControl->setModel(walletModel);
    zFlineControl->exec();
}

void PrivacyDialog::setZFlineControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzFlineSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzFLINE()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Fline Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zFLINEpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zFLINE is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " FLINE </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zFLINE</b>";
    ...
    // use mints from zFline selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZFlineControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZFlineControlDialog::GetSelectedMints();
    }
    ...
    // Clear zfline selector in case it was used
    ZFlineControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zFline Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Fline, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zFline Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zFLINE </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zFLINE "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zFLINE "));
    ui->labelzFLINEAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzFLINESyncStatus->setVisible(fShow);
}
