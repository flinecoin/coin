...
#include "ui_zflinecontroldialog.h"
...
std::list<std::string> ZFlineControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZFlineControlDialog::listMints;

ZFlineControlDialog::ZFlineControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZFlineControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZFlineControlDialog::~ZFlineControlDialog()
{
    delete ui;
}

void ZFlineControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZFlineControlDialog::updateList()
{
...
void ZFlineControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZFlineControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZFline_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZFlineControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZFlineControlDialog::GetSelectedMints()
{
...
void ZFlineControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
